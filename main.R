...
AICc <- rbind(
  fit1$aicc, fit2$aicc, fit3$aicc, fit4$aicc,
  fit5$aicc, fit6$aicc, fit7$aicc, fit8$aicc
)
BIC <- rbind(
  fit1$bic, fit2$bic, fit3$bic, fit4$bic,
  fit5$bic, fit6$bic, fit7$bic, fit8$bic
)
Modelo <- cbind(c(
  "ETS(A,A,A)", "ETS(A,Ad,A)", "ETS(M,A,A)", "ETS(M,Ad,A)",
...
p_valor <- c(
  shapiro.test(E)$p.value, kpss.test(E)$p.value,
  Box.test(E, lag = 15, type = "Ljung-Box", fitdf = 3)$p.value
)
Estatistica <- c(
  shapiro.test(E)$statistic, kpss.test(E)$statistic,
